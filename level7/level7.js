...
var winPath = ["right", "right", "up", "up", "up", "left", "down", "right", "up", "right"];

var triggerChangePath = ["right", "right", "up", "up", "up", "left"];
...
// function to check if we should trigger change
function triggerChange() {
  if (triggerChangePath.length !== userPath.length) {
    return false;
  }
  for (var i = 0; i < winPath.length; i++) {
    if (triggerChangePath[i] !== userPath[i]) {
      return false;
    }
  }
  return true;
